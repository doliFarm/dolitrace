...
/**
 * \file    test/phpunit/DoliTraceFunctionalTest.php
 * \ingroup dolitrace
 ...
 *
 * @package Testdolitrace
 */
class DoliTraceFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modDoliTrace')->click();
		} else {
			// Disable the module
			$this->byHref('modDoliTrace')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/dolitrace/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('dolitrace/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/dolitrace/admin/about.php');
		$this->authenticate();
		return $this->assertContains('dolitrace/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('dolitracewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modDoliTrace_DoliTraceTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modDoliTrace_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
