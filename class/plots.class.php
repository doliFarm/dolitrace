...
/**
 * \file        htdocs/modulebuilder/template/class/plots.class.php
 * \ingroup     dolitrace
 ...
 * Class for Plots
 */
class Plots extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'plots';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'dolifarm_plots';
	...
	/**
	 * @var string String with name of icon for plots. Must be the part after the 'object_' into object_plots.png
	 */
	public $picto = 'plots@dolitrace';
	...
	public $fields = array(
		...
		'label'         => array('type'=>'varchar(255)', 'label'=>'Label', 'enabled'=>1, 'visible'=>1, 'position'=>40, 'searchall'=>1, 'css'=>'minwidth300', 'cssview'=>'wordbreak', 'help'=>'Help text', 'showoncombobox'=>2, 'validate'=>0),
		...
		'size'         => array('type'=>'real', 'label'=>'Size', 'enabled'=>1, 'visible'=>1, 'notnull'=> 1, 'default'=>'', 'position'=>60, 'searchall'=>0, 'isameasure'=>1, 'help'=>'Help text for quantity', 'css'=>'maxwidth75imp', 'validate'=>0),
	...
	/**
	 * @var string def_sizeunit
	 */
	public $def_sizeunit;
	
	/**
	 * @var string def_organicstatus
	 */
	public $def_organicstatus;
	
	/**
	 * @var string conversionstart
	 */
	public $conversionstart;
	
	/**
	 * @var string conversionfinish
	 */
	public $conversionfinish;
	...
	//  */
	// public $table_element_line = 'dolitrace_plotsline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_plots';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Plotsline';
	...
	//  */
	// protected $childtablesoncascade = array('dolitrace_plotsdet');

	// /**
	//  * @var PlotsLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->dolitrace->plots->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'plots/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'plots/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->dolitrace->dir_output.'/plots/'.$oldref;
				$dirdest = $conf->dolitrace->dir_output.'/plots/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->dolitrace->dir_output.'/plots/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Plots").'</u>';
		...
		$url = dol_buildpath('/dolitrace/plots_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('plotsdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new PlotsLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_plots = '.((int) $this->id)));
...
/**
 * Class PlotsLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PlotsLine extends CommonObjectLine
{
	// To complete with content of an object PlotsLine
	// We should have a field rowid, fk_plots and position
