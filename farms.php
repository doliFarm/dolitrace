...
// DOLIFARM 	'typent.code'=>array('label'=>"ThirdPartyType", 'position'=>29, 'checked'=>$checkedtypetiers),
...
// DOLIFARM 	'customerorsupplier'=>array('label'=>'NatureOfThirdParty', 'position'=>61, 'checked'=>1),
...
// Add FARM where clause
/* 
 include_once DOL_DOCUMENT_ROOT.'/core/class/ctypent.class.php';
 $dictionary = new Ctypent($db);
 $farmtype = $dictionary->fetch('',"TE_FARM");      TODO Make a Dolibarr PR for $dictionary->fetch*/
...
$sql .= " AND s.fk_typent=".$farmtype->id;   // TODO(5) Search for several language
...
llxHeader('', $langs->trans("ListOfFarms"), $help_url);
...
if (!empty($type)) {
	...
	$type = 'f';     // I suppose the default is type Supplier
	$typefilter = '&amp;type='.$type;
}
...
if ($contextpage != 'poslist') {
	$url = DOL_URL_ROOT.'/societe/card.php?action=create&typent_id='.farm_typent().$typefilter;
	if (!empty($socid)) {
		$url .= '&socid='.$socid;
	}
	$newcardbutton = dolGetButtonTitle($langs->trans("NewFarm"), '', 'fa fa-plus-circle', $url, '', $user->rights->societe->creer);
	...
	$label = 'MenuNewCustomer';
	$newcardbutton .= dolGetButtonTitle($langs->trans("NewFarm"), '', 'fa fa-plus-circle', $url);
}
...
while ($i < min($num, $limit)) {
	...
	print '<div class = "row">';
	if ($conf->dolitrace->enabled) { // TODO verify hook implementation
		   // print '<br>';
...
if (!empty($socid) || !empty($search_nom)){
	...
	echo '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT."/custom/dolitrace/plots.php?search_fk_farm=".$socid.'&action=" title="'.$langs->trans('Plots').'">'.$langs->trans("Plots").'</a></div>';
}
